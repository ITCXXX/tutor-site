...
class ПризнакиПозиции(TestCase):
    ...
    def test_ни_один_признак_не_постоянен(self):
        """Признак, одинаковый во всех позициях, — не признак.

        Оценка всегда считается глазами того, чья очередь, поэтому «чей ход»
        равен единице всегда. Такой столбец работает свободным членом, которого
        по симметрии быть не должно: он означал бы, что одна из сторон
        выигрывает просто так. Один раз это уже произошло — признак «ход»
        простоял целый заход обучения и получил вес 37.8 ни за что.

        Позиции берутся из настоящей самоигры, а не из случайных ходов. Редкий
        признак иначе выглядит постоянным просто потому, что не встретился:
        ничейные поля бывают лишь в 8% позиций, и в горсти случайных их нет ни
        одного. Ошибиться так — значит выбросить рабочий признак.
        """
        строки = selfplay.набрать(партий=30, уровень='easy', ядер=1, seed=3)
        позиции = [selfplay.распаковать(у) for у, _ in строки]
        self.assertGreater(len(позиции), 500)
        значения = [features.вектор(поз, поз.turn) for поз in позиции]
        for k, имя in enumerate(features.ПРИЗНАКИ):
            встретилось = {в[k] for в in значения}
            self.assertGreater(
                len(встретилось), 1,
                'признак «%s» во всех %d позициях равен %s — это свободный '
                'член, а не признак'
                % (имя, len(позиции), встретилось.pop()))
