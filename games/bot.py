...
LEVELS = {
    ...
    'hard': {
        'title': 'сильный',
        # Пять — не порог, а место, дальше которого платить не за что.
        # Замер против этой же глубины, по 400 партий с одинаковым сроком
        # (оценка — нынешняя, обученная):
        #     глубина 2 — 36.1% ± 4.6   (явно слабее)
        #     глубина 3 — 45.9% ± 4.8
        #     глубина 4 — 47.1% ± 4.7
        #     глубина 6 — 49.6% ± 4.8
        # То есть перебор насыщается около четвёртого полухода: два лишних
        # уровня стоят четырёх процентов, один — меньше погрешности. Шестую
        # проверяли и со СНЯТЫМ сроком (бюджет 5 с) — тот же результат, так
        # что дело не в нехватке времени.
        #
        # История стоит того, чтобы её помнить. Раньше здесь стояло, что
        # глубина 6 ПРОИГРЫВАЕТ (7:13 на 20 партиях), и виновата грубость
        # оценки: перебор находит выгоду, которой на доске нет. С обученной
        # оценкой этого нет — но и прибавки нет. Пятая оставлена как запас на
        # позиции, где ветвление мало и глубина достаётся даром.
        'depth': 5,
