...
from .features import (BOARD_WEIGHT, CELL_WEIGHT, DRAW, EMPTY, LINES_THROUGH,
                       O, X, ЧИСЛО_ПРИЗНАКОВ, вектор, кортеж_весов, стадия)
...
LEVELS = {
    ...
    'hard': {
        'title': 'сильный',
        # Здесь ограничивает ВРЕМЯ, а не потолок глубины, и это выяснилось
        # не сразу. Потолки 4, 5 и 6 в замерах не отличались друг от друга —
        # все трое упирались в один и тот же срок и доходили до одной и той же
        # настоящей глубины. Отличались только 2 и 3 (36.1% и 45.9% против
        # этого уровня): их потолок ниже того, что успевает влезть.
        #
        # Сколько стоит время, замерено прямо: втрое больший срок дал
        # 53.5% ± 6.8%, вдесятеро больший — 47.5% ± 7.6%. То есть ничего:
        # перебор насыщен, и покупать ему время незачем.
        #
        # Зато его можно ПРОДАВАТЬ. Срок был 0.30 с, и живая партия через
        # настоящие вьюхи показала медиану ответа 295 мс — бот съедал бюджет
        # целиком на каждом ходу, занимая воркер. Замеры укороченного срока
        # против прежнего: 150 мс — 51.0% ± 2.8% на 1200 партиях, 80 мс —
        # 48.6% ± 4.8%, и только 40 мс проседают заметно (44.1% ± 4.8%).
        #
        # Взято 150, а не 80: срок — это стенные часы, а сервер медленнее
        # моей машины, и там те же 80 мс дадут заметно меньше узлов. Половина
        # прежней нагрузки при неотличимой игре, и ещё остаётся запас.
        'depth': 5,
        'budget': 0.15,      # и всё равно не дольше срока: ответ идёт в запросе
        'blunder': 0.0,
    },
}
...
def evaluate(pos, me, веса=None):
    ...
    Если в наборе весов есть имена с суффиксом «@номер», у каждой стадии партии
    свои веса, и здесь выбирается нужный набор. Когда стадия одна — а так было
    до обучения по стадиям — выбор ничего не стоит.
    """
    наборы = _веса_кортежем(веса if веса is not None else ВЕСА)
    w = наборы[0] if len(наборы) == 1 else наборы[стадия(pos, len(наборы))]
    в = вектор(pos, me)
    итог = 0.0
