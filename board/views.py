...
# Имя загруженного файла — 32 шестнадцатеричных знака и расширение
# (secrets.token_hex(16) при загрузке). По такому имени файл однозначно
# находится в данных объекта, как бы объект его ни хранил.
_ИМЯ_ФАЙЛА = re.compile(r'([0-9a-f]{32}\.[a-z0-9]{1,5})')


def _файлы_в_ходу(code):
    """Имена файлов, на которые ссылается хоть один объект доски.

    ЗАЧЕМ ЭТО ВООБЩЕ. Раньше файлы считались ПО ДИСКУ, и оттого счётчик умел
    только расти: удаляешь картинку с доски — объект уходит, а файл остаётся
    лежать навсегда. Владелец так и сказал: «удалял картинки, но ничего не
    менялось». Он был прав, и потолок в 60 файлов был лишь симптомом.

    Ищем имя файла ПО ВСЕМ данным объекта, а не только в поле url: картинка
    может прийти и внутри текста, и в подложке матокна, и в чём-нибудь, что
    появится завтра. Имя из 32 шестнадцатеричных знаков случайно не встретится.
    """
    from .models import BoardElement
    имена = set()
    for data in BoardElement.objects.filter(
            board__code=code).values_list('data', flat=True):
        if not data:
            continue
        имена.update(_ИМЯ_ФАЙЛА.findall(json.dumps(data, ensure_ascii=False)))
    return имена


def _board_files_usage(code):
    """Сколько файлов у доски В ХОДУ и сколько они весят.

    Файлы, на которые уже никто не ссылается, в счёт не идут — но с диска
    сразу НЕ удаляются. Так надо: отмена (Ctrl+Z) возвращает удалённый объект
    вместе с прежним адресом картинки, и если файл убрать тут же, отменённая
    картинка окажется битой. Мусор убирает отдельная команда
    `убрать_ненужные_файлы`, спустя срок, когда отменять уже некому.
    """
    d = _board_media_dir(code)
    if not d or not os.path.isdir(d):
        return 0, 0
    в_ходу = _файлы_в_ходу(code)
    n = total = 0
    for name in os.listdir(d):
        p = os.path.join(d, name)
        if os.path.isfile(p) and name in в_ходу:
            n += 1
