...
(function () {
  ...
  //
  // КЭШ ПРОВЕРЯЕТ САМ СЕБЯ, а не полагается на чужую добросовестность. Раньше
  // полагался: рамка считалась один раз и жила вечно, а сбрасывал её ровно один
  // вызов — upsertNode. Своё же перетаскивание туда не заходит: onNodeDragEnd
  // пишет data.x/y и шлёт element_update, а собственное эхо сервер отправителю
  // не возвращает (board/consumers.py). Значит после того, как ты сам подвинул
  // картинку, отсечение продолжало считать её на СТАРОМ месте — и на очередном
  // зуме прятало то, что видно. Причём только у того, кто двигал: остальным
  // обновление приходит, upsertNode срабатывает, кэш сбрасывается. Отсюда и
  // жалоба «при некотором зуме картинка перестаёт быть видной».
  //
  // Дописывать сброс в каждое место, где узел меняется, — тупик: их много, и
  ...
  // считается по ЖИВЫМ данным элемента, а не по преобразованию узла: у стрелки
  // тянут дальний конец — points другие, а x/y те же, и сверка не заметит
  // ничего. Пересчёт им обязателен. Он не бесплатный, но и не страшный: замер
  // на 2000 узлов дал 3–4 мс при бюджете кадра 16.7, а таких узлов на доске
  // горстка, и считаем мы их не чаще раза в кадр и только при зуме/панораме.
  function своя_рамка(n) {
  ...
  // Кадр собирается целиком в одном месте — панорамой (panLoop) или общей
  // перерисовкой вида (scheduleViewRedraw). Пока это идёт, помощники своей
  // отрисовки холста не заказывают: иначе холст рисуется дважды за кадр.
  let рисуемСами = false;
  ...
  function applyCull() {
    ...
    nodes.forEach((n) => {
      ...
      // Рамка схлопнулась в точку — значит измерить не вышло, а не значит, что
      // объект крошечный. Так отвечают фигуры на sceneFunc, которым мы не дали
      // своей getSelfRect: пометки, углы, коники, полуокружности и ГРАФИКИ
      // ФУНКЦИЙ в матокне. Konva для них возвращает {0,0,0,0} — точку в начале
      // мировых координат, где бы фигура ни рисовалась. Проверка тогда
      // вырождалась в «начало координат на экране?», и график пропадал, стоило
      // отъехать от нуля: заметнее всего при ПРИБЛИЖЕНИИ, потому что видимый
      // кусок мира сжимается. Отсечение — ускорение, а не право прятать: чего
      // не измерили, то показываем.
      const вне = (b.width > 0 || b.height > 0)
        && (b.x > x1 || b.y > y1 || b.x + b.width < x0 || b.y + b.height < y0);
      if (!!n._culled !== вне) { n._culled = вне; setNodeShown(n); менялось = true; }
    });
    // Пока кадр собирает панорама (признак объявлен выше), своей отрисовки не
    // заказываем: она нарисует всё разом и синхронно, в конце кадра.
    if (менялось && !рисуемСами) layer.batchDraw();
  }
  ...
  function scheduleViewRedraw() {
    ...
    viewRAF = requestAnimationFrame(() => {
      ...
      try {
        ...
        positionHandles();
        renderAnchors();
      } finally { рисуемСами = false; }
      // Одна отрисовка холста на кадр — и за отсечение, и за ручки, и за
      // положение текста со стикерами поверх холста: оно привязано к событию
      // отрисовки главного слоя, и без этой строки зависело бы от того, когда
      // соберётся рисовать сама Konva.
      layer.draw();
    });
  }

  // ── Доска встаёт на целую точку экрана ────────────────────────────────
  // Текст, формулы, стикеры и таблицы на доске — это не рисунок на холсте, а
  // настоящие буквы поверх него, и ставятся они сдвигом. Сдвиг на ДРОБНОЕ
  // число точек заставляет браузер каждый кадр заново вырисовывать буквы с
  // новой полутоновой каймой по краям — глазу это видно как «плавание» текста
  // во время движения доски. Само положение при этом верное: замерено по
  // кадрам, холст и текст в каждом кадре стоят в одной точке, расхождение
  // ровно ноль.
  //
  // Поэтому доска ездит по целым ФИЗИЧЕСКИМ точкам экрана (их на нынешних
  // экранах две-три на каждую условную). Тогда за время движения дробная часть
  // положения у всего, что на доске лежит, не меняется вовсе: буквы переезжают
  // готовыми, а холст перерисовывается с той же раскладкой полутонов — дрожать
  // нечему.
  //
  // Округлять положение сцены «в лоб» было бы нельзя: медленное движение тогда
  // пропадает совсем. Палец, ползущий по трети точки за раз, каждый раз
  // округлялся бы обратно, и доска стояла бы намертво. Поэтому желаемое
  // положение хранится отдельно и БЕЗ округления, а округляется только то, что
  // записывается в сцену.
  function кТочкеЭкрана(v) {
    const плотность = window.devicePixelRatio || 1;
    return Math.round(v * плотность) / плотность;
  }
  let _видЦель = null;     // куда доску просили — без округления
  let _видПисали = null;   // что мы записали в сцену в прошлый раз
  // Доску мог подвинуть и кто-то другой: зум, центрирование, «показать всё»,
  // вид от ведущего. Тогда наше накопленное желаемое положение устарело, и
  // отсчёт надо начинать от того, что в сцене сейчас. Видно это по расхождению
  // с тем, что мы писали сами, — отдельного оповещения не требуется.
  function _видОснова() {
    if (!_видЦель || !_видПисали || _видПисали.x !== stage.x() || _видПисали.y !== stage.y()) {
      _видЦель = { x: stage.x(), y: stage.y() };
    }
    return _видЦель;
  }
  function _видЗаписать() {
    _видПисали = { x: кТочкеЭкрана(_видЦель.x), y: кТочкеЭкрана(_видЦель.y) };
    stage.position(_видПисали);
  }
  // Сдвинуть доску НА столько-то (палец, колесо, правая кнопка, выкат).
  function сдвинутьВидНа(dx, dy) {
    const ц = _видОснова();
    ц.x += dx; ц.y += dy;
    _видЗаписать();
  }
  // Поставить доску В точку (перетаскивание в режиме перемещения: там сдвиг
  // считается от места нажатия целиком, и копить нечего).
  function сдвинутьВидК(x, y) {
    _видЦель = { x: x, y: y };
    _видЗаписать();
  }
  ...
  function syncStageSize() {
    ...
    redrawGrid();
    repositionCursors();
    // Коробка отсечения считается от размера сцены — значит она сейчас
    // устарела. Без пересчёта объект, попавший в кадр из-за того, что окно
    // стало больше, остаётся спрятанным до ближайшего зума или панорамы:
    // повернули планшет — и половины доски нет, пока не тронешь вид.
    applyCull();
  ...
  function upsertNode(el) {
    ...
    // Сброс остаётся и здесь: сверка преобразования не заметит смену ТОЧЕК
    // ломаной, содержимого группы или толщины обводки — а рамку они меняют.
    //
    // И ОБЯЗАТЕЛЬНО зажечь узел обратно. Выше по этой же функции
    // applyElVisibility зовёт setNodeShown, а тот читает _culled — ещё СТАРЫЙ.
    // Значит объект, который был за кадром, получал visible(false), и только
    // потом флаг обнулялся. Узел оставался в невозможном состоянии: «не
    // отсечён, но невидим». Починить его было уже некому: applyCull меняет
    // видимость только на ПЕРЕЛОМЕ (`if (!!n._culled !== вне)`), а тут обе
    // стороны false — перелома нет, и картинка не возвращалась до перезагрузки
    // страницы. Ловилось это так: сосед подправил объект, пока тот был у тебя
    // за краем экрана.
    if (node) {
  ...
  // Насколько пальцы должны разъехаться, чтобы это считалось щипком, а не
  // возкой доски: четыре процента расстояния между ними (при пальцах в ладони
  // друг от друга — пара миллиметров). Меньшее — дрожание руки.
  const ЩИПОК_ПОРОГ = 0.04;   // в логарифмической мере, |ln k|

  function startGesture() {
    const c = touchCenter();
    if (!c) return;
    // зумЖдёт — «жест пока считается возкой, масштаб не трогаем». Снимается
    // один раз за жест, когда пальцы разъедутся заметно.
    gesture = { cx: c.x, cy: c.y, dist: touchSpread(), zoomAcc: 0, зумЖдёт: true, зумКадр: null };
  ...
  function updateGesture() {
    ...
    if (dx || dy) сдвинутьВидНа(dx, dy);
    gesture.cx = c.x; gesture.cy = c.y;
    отметитьРазгон(c.x, c.y);   // след для инерции: где была рука и когда
    // ЩИПОК ОТДЕЛЁН ОТ ВОЗКИ ДОСКИ. Вести два пальца по стеклу, сохраняя между
    // ними расстояние, человек не может: оно гуляет на проценты. Прежний порог
    // — 0,2 % за одно событие — это меньше половины точки при пальцах в ладони
    // друг от друга, то есть обычное дрожание руки. Поэтому при простой возке
    // доски двумя пальцами масштаб менялся чуть ли не каждый кадр: доска
    // незаметно «дышала». Рисунку это почти незаметно (он вычерчивается
    // заново), а текст, формулы, стикеры и таблицы — настоящие буквы поверх
    // холста, и браузер рисует их заново при каждом новом размере. Со стороны
    // это и выглядит как плавающий текст.
    //
    // Теперь, пока пальцы не разъехались на ЩИПОК_ПОРОГ, масштаб держится
    // НЕИЗМЕННЫМ, а жест считается возкой. Как только разъехались — отсчёт
    // начинается заново от этого мига: скачка на эти четыре процента не будет,
    // они и есть мёртвая зона.
    //
    // Масштаб считается РАЗ В КАДР, а не на каждое событие, и вот почему.
    // События от пальцев приходят по одному: сперва сдвинулся первый палец,
    // потом второй. Между этими двумя событиями расстояние между пальцами
    // отличается от настоящего ровно на длину шага — при возке доски на 13
    // точек оно скачет 200 → 187 → 200 → 187. По одному событию щипок от возки
    // не отличить в принципе. К началу же кадра оба пальца уже на новых
    // местах, и расстояние между ними настоящее: у чистой возки оно не
    // меняется вовсе.
    if (gesture.зумКадр == null) {
      gesture.зумКадр = requestAnimationFrame(() => {
        if (gesture) gesture.зумКадр = null;
        проверитьЩипок();
      });
    }
    scheduleViewRedraw();   // перерисовать сетку/курсоры и передать вид ведомым
  }
  // Щипок: масштаб вокруг середины между пальцами — точка под пальцами стоит
  // на месте. Зовётся кадром из updateGesture, когда оба пальца уже сдвинулись.
  function проверитьЩипок() {
    if (!gesture) return;
    const c = touchCenter(); if (!c) return;
    const d = touchSpread();
    if (!(d > 0)) return;
    if (!(gesture.dist > 0)) { gesture.dist = d; return; }
    const k = d / gesture.dist;
    if (gesture.зумЖдёт) {
      // Пока пальцы не разъехались на ЩИПОК_ПОРОГ, жест считается возкой и
      // масштаб держится неизменным. Разъехались — отсчёт с этого мига, чтобы
      // не было скачка на величину мёртвой зоны.
      if (Math.abs(Math.log(k)) > ЩИПОК_ПОРОГ) { gesture.зумЖдёт = false; gesture.dist = d; }
      return;
    }
    if (Math.abs(k - 1) > 0.002) {
      // Копим ВЕЛИЧИНУ зума за жест, а не просто «зум был»: по ней потом
      // решается, был это щипок (тогда выката нет) или бросок. Мелочь ниже
      // 0,2 % не выбрасываем, а оставляем накапливаться — иначе медленный
      // щипок не сдвинул бы масштаб вовсе.
  ...
  function endGestureIfDone() {
    // Заказанную проверку щипка снимаем: жест кончился, считать нечего.
    if (touchPts.size === 0 && gesture && gesture.зумКадр != null) {
      cancelAnimationFrame(gesture.зумКадр); gesture.зумКадр = null;
    }
    if (touchPts.size === 0) {
  ...
  // Собираем кадр сами — холст нарисуют один раз в конце, и ручки попадут
  // в ту же отрисовку. Замерено в браузере: во время панорамы главный слой
  // рисовался за кадр дважды — сперва тем, кто собирает кадр, следом отсюда.
  function рисоватьРучки() { if (рисуемСами) return; if (тащатОбъект) layer.batchDraw(); else layer.draw(); }
  ...
  function panBoardBy(dx, dy) {
    следованиеОтпустить();
    сдвинутьВидК(panDrag.sx + dx, panDrag.sy + dy);
  ...
  // Открытая подпанель ВСЕГДА ОДНА. Раньше чужие никто не закрывал: панель
  // нужной группы просто открывалась поверх. Нажал L при открытых карандашах —
  // панель линий легла сверху, а карандаши остались висеть; то же самое с любой
  // другой горячей клавишей и с выбором инструмента из меню.
  function закрытьПодпанели(кроме) {
    document.querySelectorAll('#board-toolbar .tool-flyout.open').forEach((f) => {
      if (f !== кроме) f.classList.remove('open');
    });
  }
  function syncDrawFlyout() {
    ...
    // Инструмент рисующий — оставляем открытой только его панель (там толщина и
    // цвет). Любой другой — закрываем все: его настройки живут не здесь.
    закрытьПодпанели(isDrawToolActive() ? e.fly : null);
    if (isDrawToolActive()) {
      if (cfg) {
        cfg.hidden = !key;
  ...
  function fitAllToView() {
    ...
    // applyCull здесь ОБЯЗАТЕЛЕН: «показать всё» приводит в кадр объекты,
    // которые до этого были за его пределами и потому спрятаны отсечением.
    // Без этой строки они оставались невидимыми до следующей панорамы — доска
    // показывала пустоту там, где всё на месте.
    applyCull();
  ...
  // Было 750 — владелец попросил выше. С Shift вдвое: 2400.
  const PAN_SPEED = 1200;  // макс. скорость, px/сек
  ...
  // Потолок скорости броска. Было 2500, и быстрый бросок пальцем просто
  // срезался — владелец: «повысь максимальную скорость». При 6000 сильный бросок
  // уносит доску примерно на два с половиной экрана.
  const ИНЕРЦИЯ_ПОТОЛОК = 6000;   // px/с
  // Затухание: чем больше, тем короче выкат. Было 5, и выкат выходил в два с
  // половиной раза короче привычного — владелец: «инерция маленькая». 2 — как у
  // прокрутки на iPhone: скорость гаснет на две десятых процента за
  // миллисекунду (−ln 0,998 · 1000 ≈ 2). Путь выката ≈ скорость / 2.
  const ИНЕРЦИЯ_ЛЯМБДА = 2;       // 1/с
  const ИНЕРЦИЯ_СТОП = 30;        // px/с: ниже — считаем, что доска встала
  // Предел выката по времени. С мягким затуханием самый сильный бросок катится
  // ≈ 2,7 с, и прежний предел 1,2 с обрезал бы его на середине. Это страховка для
  // слабых устройств, на обычных до неё не доходит.
  const ИНЕРЦИЯ_МАКС_МС = 4000;   // мс
  ...
  function panLoop(t) {
    ...
    if (выбег) {
      ...
      // пять-шесть секунд после того, как её отпустили, — это не инерция, это
      // выглядит поломкой.
    ...
    // Порог остановки у выката выше: хвост от тридцати пикселей в секунду до
    // одного тянется ещё без малого две секунды, доска за него проезжает около
    // пятнадцати пикселей — глазом не видно, а каждый из этих кадров стоит
    // полной перерисовки.
    ...
    сдвинутьВидНа(panVX * dt, panVY * dt);
    ...
    // Сначала ВСЁ, что меняет картинку, и только потом единственная отрисовка.
    // Раньше холст рисовался в середине, а ручки размера двигались уже после
    // него: они отставали на кадр, и на их перемещение уходила ещё одна полная
    // отрисовка холста. Замерено в браузере: было две отрисовки за кадр, стала
    // одна.
    рисуемСами = true;
    try {
      applyCull();
  ...
  // ── DOM-слой едет ВМЕСТЕ с холстом ────────────────────────────────────
  // Всё, что лежит поверх холста обычными элементами страницы — текст,
  // стикеры, таблицы, таймеры, ползунки, встроенные страницы, чужие курсоры,
  // якоря стрелок, — здесь ставится по текущему виду доски.
  //
  // Зовётся из события отрисовки слоя (подписка сразу ниже). Раньше это делали
  // сами пути движения — каждый по-своему и в свой момент, — и половины доски
  // расходились на кадр. Теперь связь одна и на всех: нарисовался холст, значит
  // DOM уже на месте.
  let _видПрошлый = '';
  function положитьНадХолстом() {
    // Сама перестановка идёт БЕЗ УСЛОВИЙ, на каждую отрисовку. Сначала я хотел
    // делать её только при смене вида — дешевле, — но тогда мимо проходит
    // второй случай рассинхрона: тащат выделение, где вместе стикер и чертёж.
    // Стикер двигается сразу (это DOM), чертёж — со следующей отрисовкой, и
    // они опять расходятся. А стоит перестановка нескольких присвоений строки
    // в style, так что экономить тут не на чем.
    if (typeof положитьВиджеты === 'function') положитьВиджеты();
    ...
    // и так обновляет тот, кто тащит.
    const ключ = stage.x() + ',' + stage.y() + ',' + stage.scaleX();
    if (ключ !== _видПрошлый) {
      _видПрошлый = ключ;
      if (typeof renderAnchors === 'function') renderAnchors();
    }
  }
  layer.on('draw', function () { положитьНадХолстом(); });
  ...
  function applyView(user, x, y, scale) {
    ...
    // Масштаб пришёл по сети, а значит из чужих рук. Свой зум ограничен
    // (zoomTo зажимает в MIN_SCALE..MAX_SCALE), а этот путь — нет: ноль,
    // отрицательное или NaN схлопывали ведомому всю доску в ничто, и выбраться
    // он мог только сам, вручную. Два пути к одному и тому же масштабу должны
    // подчиняться одному правилу.
    const s0 = Number(scale);
    const безопасный = Number.isFinite(s0)
  ...
  function watchLevel(holder, stream) {
    try {
      голосКонтекст();
      // Прежний источник отключаем: при переподключении микрофона и новом потоке
      // собеседника они иначе копились бы в контексте до конца занятия.
      if (holder.levelSrc) { try { holder.levelSrc.disconnect(); } catch (e) {} holder.levelSrc = null; }
  ...
  document.addEventListener('click', () => {
    // Громкость выше 100% сохранилась с прошлого занятия, а щелчка на странице ещё
    // не было — движок усиления стоит на паузе. Будим: пока он стоит, голос идёт
    // обычным путём, без надбавки.
    звукРазбудить();
    if (!audioBlocked) return;
    audioBlocked = false;
    voicePeers.forEach((p) => { p.needsClick = false; if (p.audio) { const g = p.audio.play(); if (g && g.catch) g.catch(() => {}); } });
    if (громче.audio) { const g = громче.audio.play(); if (g && g.catch) g.catch(() => {}); }
    renderVoiceList();
  }, true);
  ...
  function closePeer(pid) {
    ...
    разобратьУсиление(p);
    if (p.levelSrc) { try { p.levelSrc.disconnect(); } catch (e) {} p.levelSrc = null; }
    if (p.audio) { try { p.audio.srcObject = null; } catch (e) {} p.audio.remove(); }
    voicePeers.delete(pid);
    if (!естьСлушать()) громчеРазобратьПозже();
  ...
  function voiceStart() {
    ...
    navigator.mediaDevices.getUserMedia(MIC_REQ).then((stream) => {
      ...
      // И новая попытка петли усиления, если она раньше не собралась. Включение
      // микрофона всегда даёт одну попытку: без микрофона эха не было, и срывы до
      // него не должны её отнять.
      if (громче.попыток >= 3) громче.попыток = 2;
      петляЕщё();
      updateVoiceUI();
  ...
  //
  // ЭХО. Подавитель эха вычитает из микрофона звук, который играет компьютер, —
  // но только тот, который он видит. Chrome на компьютере с 2023 года видит весь
  // свой звук. А Chrome на Android, ChromeOS без системного подавителя, Chrome с
  // выключенным флагом «эхоподавление на весь Chrome» видят только голос
  // собеседников, играющий обычным audio-элементом. Звук, выведенный в колонки
  // прямо из Web Audio, им не виден: собеседник слышал бы своё эхо. Поэтому
  // усиленный голос уходит в колонки ТОЖЕ audio-элементом — через «петлю»: пару
  // WebRTC-соединений внутри этой же страницы. Для подавителя это обычный голос
  // собеседника. Цена — около 75 мс задержки (замер), и только выше 100%.
  ...
  // Движок встал (браузер приостановил звук страницы) — усиленный голос молчал бы,
  // поэтому громкость пересчитываем: пока он стоит, голос идёт обычным путём.
  function голосКонтекст() {
    if (!voiceCtx) {
      voiceCtx = new (window.AudioContext || window.webkitAudioContext)();
      voiceCtx.onstatechange = () => {
        if (voiceCtx.state === 'running') звукРазбудить();
        applyVoiceVolume();
      };
    }
  ...
  const громче = {
    сумма: null,          // GainNode 0.5: запас по уровню перед ограничителем
    ограничитель: null,   // мягкий ограничитель (WaveShaper)
    dest: null,           // вход петли
    pcA: null, pcB: null, // петля: два соединения внутри страницы
    audio: null,          // элемент, который играет усиленный голос
    режим: 'нет',         // 'нет' | 'собирается' | 'петля' | 'прямо'
    номер: 0,             // номер сборки петли: запоздалые ответы старой выбрасываем
    таймерСборки: null, таймерРазбора: null,
    выход: 'нет',         // куда сейчас подключён ограничитель
    попыток: 0,           // повторных сборок петли после сбоя
    // Сторож петли: уровень, поданный в петлю, и уровень, вышедший из неё.
    anВход: null, bufВход: null, srcПетля: null, anПетля: null, bufПетля: null,
    сторож: null, тихо: 0, хорошо: 0, сорвалось: null,
    подсказаноIOS: false, // подсказка «на iPhone громче 100% — с включённым голосом» уже была
    быстрыйБыл: false,    // быстрый повтор после «не соединилась» уже потрачен (см. петляСобрать)
  };

  // Мягкий ограничитель. На вход подаём сигнал, уменьшенный вдвое (узел «сумма»),
  // поэтому точка кривой u — это настоящий уровень 2u. До 0,7 звук проходит как
  // есть, выше плавно прижимается к 0,9: при усилении ×2 громкие места не
  // срезаются в треск, а искажённое эхо подавитель гасит хуже чистого. Потолок
  // не 1, а 0,9 — запас на кодек петли: с потолком 1 пики после петли упирались
  // ровно в 1 (замер: синус 0,6 при громкости 200%).
  function криваяОграничителя() {
    const n = 2049, c = new Float32Array(n);
    for (let i = 0; i < n; i++) {
      const x = 2 * (2 * i / (n - 1) - 1), a = Math.abs(x);
      c[i] = a <= 0.7 ? x : Math.sign(x) * (0.7 + 0.2 * Math.tanh((a - 0.7) / 0.2));
    }
    return c;
  }
  function громчеУзлы() {
    if (громче.сумма) return;
    const ctx = голосКонтекст();
    const сумма = ctx.createGain();
    сумма.gain.value = 0.5;
    const огр = ctx.createWaveShaper();
    огр.curve = криваяОграничителя();
    сумма.connect(огр);
    const anВход = ctx.createAnalyser();
    anВход.fftSize = 4096;
    // Вход сторож слушает с опозданием на 0,2 с — почему, сказано в петляСторож.
    const задержка = ctx.createDelay(1);
    задержка.delayTime.value = 0.2;
    сумма.connect(задержка); задержка.connect(anВход);
    громче.сумма = сумма; громче.ограничитель = огр;
    громче.anВход = anВход; громче.bufВход = new Float32Array(anВход.fftSize);
  }
  // Куда уходит усиленный звук: в петлю, прямо в колонки или никуда.
  function громчеВыход(куда) {
    if (!громче.ограничитель) return;
    if (куда === 'петля' && !громче.dest) куда = 'нет';
    if (куда !== 'петля' && куда !== 'прямо') куда = 'нет';
    // Граф перестраиваем, только когда выход правда меняется: отключение и
    // подключение — две отдельные правки, звуковой поток может успеть проиграть
    // между ними блок тишины — щелчок посреди голоса.
    if (громче.выход === куда) return;
    try { громче.ограничитель.disconnect(); } catch (e) {}
    if (куда === 'петля') громче.ограничитель.connect(громче.dest);
    else if (куда === 'прямо') громче.ограничитель.connect(voiceCtx.destination);
    громче.выход = куда;
  }
  // Прямой вывод на iPhone и iPad — только пока захвачен свой микрофон. Без него
  // iOS считает звук страницы фоновым, и переключатель «без звука» глушит Web
  // Audio целиком, а заглушённый элемент не звучит — была бы тишина. На Mac
  // такого переключателя нет, там прямой вывод можно всегда.
  function громчеЯблокоМобильное() {
    const ua = navigator.userAgent || '';
    return /iPhone|iPad|iPod/.test(ua) || (/Macintosh/.test(ua) && navigator.maxTouchPoints > 1);
  }
  function громчеПрямоМожно() { return !громчеЯблокоМобильное() || !!(voiceOn && localStream); }
  // Куда сейчас действительно может уйти усиленный звук.
  function громчеКуда() {
    if (!voiceCtx || voiceCtx.state !== 'running' || !громче.ограничитель) return 'нет';
    if (громче.режим === 'петля') return 'петля';
    if (громче.режим === 'прямо' && громчеПрямоМожно()) return 'прямо';
    return 'нет';
  }
  // Усиленный звук сейчас действительно звучит (иначе исходный элемент глушить
  // нельзя). Для петли — ещё и её элемент играет: браузер мог не пустить звук без
  // действия человека, и тогда голос должен идти обычным путём.
  function громчеСлышно() {
    const куда = громчеКуда();
    if (куда === 'прямо') return true;
    return куда === 'петля' && !!громче.audio && !громче.audio.paused;
  }
  // Есть ли кого слушать: собеседник, чей голос пришёл.
  function естьСлушать() { return Array.from(voicePeers.values()).some((p) => p.audio); }
  // Петлю разбираем не сразу: ползунок часто двигают туда-сюда через 100%, а
  // собеседник, вышедший из голоса, нередко тут же входит снова.
  function громчеРазобратьПозже() {
    if (громче.режим === 'нет' || громче.таймерРазбора) return;
    громче.таймерРазбора = setTimeout(() => {
      громче.таймерРазбора = null;
      if (voiceVolume <= 1 || !естьСлушать()) { петляЗакрыть(); громче.попыток = 0; громче.быстрыйБыл = false; }
    }, 10000);
  }
  // После сбоя петли прямой вывод не держим до конца урока: пробуем собрать её
  // снова, когда что-то могло измениться, — включён свой микрофон (адреса у
  // браузера становятся настоящими), связь с собеседником восстановилась,
  // вернулась сеть. Не больше трёх попыток до разбора петли: там, где она не
  // соберётся никогда (VPN, политика), громкость не должна проседать раз за
  // разом — пока петля собирается, голос звучит без надбавки.
  function петляЕщё() {
    if (громче.режим !== 'прямо' || шумЯблоко() || !window.RTCPeerConnection) return;
    // Громкость ненадолго убавили до 100% и ниже — повтор не теряем, а откладываем.
    // Если убавили надолго, петлю разберут, режим станет 'нет', и повтор затихнет.
    if (voiceVolume <= 1) { setTimeout(петляЕщё, 30000); return; }
    if (громче.попыток >= 3) return;
    громче.попыток++;
    громче.режим = 'нет';
    applyVoiceVolume();
  }
  // Разрешение на звук браузер даёт по любому действию человека. Будим движок
  // усиления (он мог уйти в паузу или «прерывание» — звонок, уход в фон) и
  // запускаем элемент петли, если браузер его не пустил.
  function звукРазбудить() {
    if (voiceVolume > 1 && voiceCtx && voiceCtx.state !== 'running' && voiceCtx.state !== 'closed') voiceCtx.resume().catch(() => {});
    if (громче.режим === 'петля' && громче.audio && громче.audio.paused) {
      const g = громче.audio.play();
      if (g && g.catch) g.catch(() => {});
    }
  }
  // Подготовить усиление: общие узлы, петлю, выход. Можно звать сколько угодно раз.
  function громчеГотовить() {
    if (voiceVolume <= 1) return;
    if (громче.таймерРазбора) { clearTimeout(громче.таймерРазбора); громче.таймерРазбора = null; }
    try {
      громчеУзлы();
      if (громче.режим === 'нет') {
        // Технику Apple не трогаем: проверить нечем, а несколько audio-элементов и
        // Web Audio с WebRTC на iPhone исторически капризны. Там как было — прямо.
        if (шумЯблоко() || !window.RTCPeerConnection) громче.режим = 'прямо';
        else петляСобрать();
      }
    } catch (e) {
      if (громче.режим !== 'петля') { петляЗакрыть(); громче.режим = 'прямо'; }
    }
    // На iPhone и iPad без своего микрофона надбавки нет (см. громчеПрямоМожно) —
    // говорим об этом один раз, чтобы 200% на ползунке не вводили в заблуждение.
    if (громче.режим === 'прямо' && !громчеПрямоМожно() && !громче.подсказаноIOS) {
      громче.подсказаноIOS = true;
      boardHint('Громче 100% на iPhone и iPad звучит, когда включён ваш голос', 6000);
    }
    громчеВыход(громчеКуда());
  }

  function ждатьАдресов(pc) {
    return new Promise((готово) => {
      if (pc.iceGatheringState === 'complete') { готово(); return; }
      // Не дольше 2 с на сторону: без части адресов петля внутри компьютера
      // всё равно соединяется, а ждать дольше — держать голос без надбавки.
      const т = setTimeout(готово, 2000);
      pc.addEventListener('icegatheringstatechange', () => {
        if (pc.iceGatheringState === 'complete') { clearTimeout(т); готово(); }
      });
    });
  }
  function петляСобрать() {
    const номер = ++громче.номер;
    громче.режим = 'собирается';
    const безМикрофона = !(voiceOn && localStream);
    // Петля не вышла — усиленный голос идёт прямо в колонки, как до этой правки:
    // звук важнее, чем эхо на части устройств.
    const сорвалось = (почему, повторЧерез) => {
      if (номер !== громче.номер) return;
      try { console.warn('голос: петля усиления не работает, звук идёт прямо —', почему); } catch (_) {}
      петляЗакрыть();
      громче.режим = 'прямо';
      applyVoiceVolume();
      // Сбой мог быть случайным (компьютер был занят) — ещё одна попытка через
      // полминуты; число попыток ограничено в петляЕщё. Если собирать начали ещё
      // без своего микрофона, а сорвалось уже при нём, пробуем сразу: адреса у
      // браузера теперь настоящие, а звук тем временем идёт мимо эхоподавителя.
      // Сбой, который лечится быстрым повтором, называет свой срок сам (предел на
      // соединение ниже). Быстрый повтор — один раз подряд: не соединилась и
      // вторая пара — дело не в случайности (расширение браузера, политика сети),
      // а частые пересборки дали бы качели: пока петля собирается, голос звучит
      // без надбавки. Право на быстрый повтор возвращает собранная петля.
      let через = 30000;
      if (безМикрофона && voiceOn && localStream) через = 0;
      else if (повторЧерез != null && !громче.быстрыйБыл) { через = повторЧерез; громче.быстрыйБыл = true; }
      setTimeout(петляЕщё, через);
    };
    громче.сорвалось = сорвалось;
    let pcA, pcB;
    try {
      if (!громче.dest) громче.dest = voiceCtx.createMediaStreamDestination();
      pcA = new RTCPeerConnection();
      pcB = new RTCPeerConnection();
    } catch (e) { сорвалось(e); return; }
    const a = document.createElement('audio');
    a.autoplay = true; a.playsInline = true;
    const коробка = document.getElementById('voice-audio');
    if (коробка) коробка.appendChild(a);
    громче.pcA = pcA; громче.pcB = pcB; громче.audio = a;
    // Заиграл или встал элемент петли — пересчитать, кого глушить.
    a.onplaying = a.onpause = () => { if (номер === громче.номер) applyVoiceVolume(); };

    const принять = () => {
      if (номер !== громче.номер || громче.режим !== 'собирается') return;
      if (громче.таймерСборки) { clearTimeout(громче.таймерСборки); громче.таймерСборки = null; }
      громче.режим = 'петля';
      try {
        громче.srcПетля = voiceCtx.createMediaStreamSource(a.srcObject);
        громче.anПетля = voiceCtx.createAnalyser();
        громче.anПетля.fftSize = 16384;   // вчетверо длиннее окна входа — см. петляСторож
        громче.srcПетля.connect(громче.anПетля);
        громче.bufПетля = new Float32Array(громче.anПетля.fftSize);
      } catch (_) {}
      громче.тихо = 0; громче.хорошо = 0; громче.быстрыйБыл = false;
      громче.сторож = setInterval(петляСторож, 1000);
      applyVoiceVolume();
    };
    const проверить = () => {
      if (номер !== громче.номер || громче.режим !== 'собирается') return;
      if (pcB.connectionState !== 'connected' || !a.srcObject) return;
      const go = a.play();
      if (!go || !go.then) { принять(); return; }
      go.then(принять, (e) => {
        if (e && e.name === 'NotAllowedError') {
          // Браузер не пускает звук без щелчка — как у собеседников: подсказка, а
          // петля заиграет с первого щелчка (его обработчик зовёт play).
          audioBlocked = true;
          boardHint('Браузер не пускает звук — щёлкните по доске, чтобы услышать');
          принять();
        } else {
    ...
    pcB.ontrack = (e) => {
      // Буфер у приёмника — поменьше: петля живёт внутри одного компьютера, сеть не
      // дёргается, и лишняя задержка ни к чему.
    ...
    pcB.onconnectionstatechange = () => {
      const st = pcB.connectionState;
      if (st === 'connected') проверить();
      // 'disconnected' бывает и мимолётным (перегруженный компьютер) — не сбой:
      // если петля правда замолчала, её поймает сторож.
      else if (st === 'failed' || st === 'closed') сорвалось('соединение ' + st);
    };
    pcA.onconnectionstatechange = () => {
      const st = pcA.connectionState;
      if (st === 'failed' || st === 'closed') сорвалось('соединение ' + st);
    };
    // Общий предел на всю сборку. Отдельный предел на само соединение ставится,
    // когда стороны обменялись описаниями: ожидание адресов (до 2 с на сторону)
    // не должно съедать время соединения — так петля и срывалась.
    громче.таймерСборки = setTimeout(() => { if (громче.режим === 'собирается') сорвалось('не собралась за 12 с'); }, 12000);
    const sender = pcA.addTrack(громче.dest.stream.getAudioTracks()[0], громче.dest.stream);
    // Описание связи передаём ЦЕЛИКОМ, дождавшись адресов. По одному адреса
    // приходили раньше, чем у второй стороны было описание, и петля не соединялась.
    pcA.createOffer()
      .then((o) => pcA.setLocalDescription(o)).then(() => ждатьАдресов(pcA))
      .then(() => pcB.setRemoteDescription(pcA.localDescription))
      .then(() => pcB.createAnswer()).then((o) => pcB.setLocalDescription(o)).then(() => ждатьАдресов(pcB))
      .then(() => pcA.setRemoteDescription(pcB.localDescription))
      .then(() => {
        if (номер !== громче.номер || громче.режим !== 'собирается') return null;
        if (громче.таймерСборки) clearTimeout(громче.таймерСборки);
        // Внутри одного компьютера соединение встаёт за миллисекунды. Но замер в
        // Chrome: изредка (3 сборки из 24) новая пара так и не начинает проверять
        // связь, а такая же пара через секунду соединяется сразу. Поэтому после
        // этого сбоя повтор — через секунду, а не через полминуты.
        громче.таймерСборки = setTimeout(() => { if (громче.режим === 'собирается') сорвалось('не соединилась за 5 с', 1000); }, 5000);
        return null;
      })
      .then(() => {
        // Голос кодируется заново — даём кодеку с запасом, чтобы не терять качество.
        try {
          const prm = sender.getParameters();
          if (prm.encodings && prm.encodings[0]) { prm.encodings[0].maxBitrate = 128000; return sender.setParameters(prm).catch(() => {}); }
        } catch (_) {}
        return null;
      })
      .catch((e) => сорвалось(e));
  }
  // Сторож петли. Соединение внутри страницы может умереть молча (замер:
  // закрыли вторую сторону — событие не пришло, исходный элемент заглушён, петля
  // молчит). Раз в секунду сравниваем, сколько звука подали в петлю и сколько из
  // неё вышло; три замера «подаём, а не выходит» без выхода звука между ними —
  // прямой вывод.
  function петляСторож() {
    if (громче.режим !== 'петля' || !voiceCtx || voiceCtx.state !== 'running' || !громче.anПетля) return;
    // Элемент петли на паузе (браузер не пустил звук) — голос и так идёт обычным
    // путём, судить не о чем.
    if (!громче.audio || громче.audio.paused) return;
    const вход = громкость(громче.anВход, громче.bufВход) * 2;   // «сумма» делит на два
    // Подали тишину — судить не по чему. Счётчик не обнуляем: иначе короткие
    // реплики с паузами прятали бы умершую петлю.
    if (вход <= 0.01) return;
    const выход = громкость(громче.anПетля, громче.bufПетля);
    // Звук выходит из петли позже, чем входит (кадр кодека, буфер приёмника —
    // 50–100 мс, на телефоне бывает больше). Поэтому вход слушаем с опозданием на
    // 0,2 с (узел задержки в громчеУзлы): 85 мс входа, поданные 0,2–0,3 с назад,
    // при задержке петли до 0,2 с целиком лежат в окне выхода (340 мс). Без этого
    // начало реплики — вход уже звучит, а выход ещё нет — считалось бы «молчит».
    if (выход < вход * 0.1) {
      if (++громче.тихо >= 3 && громче.сорвалось) громче.сорвалось('петля молчит');
    } else {
      громче.тихо = 0;
      // Петля доказала, что работает: после минуты звучащей речи прощаем прежние
      // срывы — иначе после нескольких за урок новые попытки кончились бы.
      if (++громче.хорошо >= 60) { громче.хорошо = 0; громче.попыток = 0; }
    }
  }
  function петляЗакрыть() {
    громче.номер++;
    if (громче.сторож) { clearInterval(громче.сторож); громче.сторож = null; }
    if (громче.srcПетля) { try { громче.srcПетля.disconnect(); } catch (e) {} }
    громче.srcПетля = null; громче.anПетля = null; громче.bufПетля = null; громче.сорвалось = null;
    if (громче.таймерСборки) { clearTimeout(громче.таймерСборки); громче.таймерСборки = null; }
    [громче.pcA, громче.pcB].forEach((pc) => {
      if (!pc) return;
      pc.onconnectionstatechange = null; pc.ontrack = null;
      try { pc.close(); } catch (e) {}
    });
    if (громче.audio) {
      try { громче.audio.pause(); громче.audio.srcObject = null; } catch (e) {}
      громче.audio.remove();
    }
    громче.pcA = null; громче.pcB = null; громче.audio = null;
    // Режим сбрасываем здесь же: иначе ушедшая петля (pagehide) числилась бы
    // живой. Кто закрывает из-за сбоя, сам ставит 'прямо' после этого.
    громче.режим = 'нет';
  }

  function разобратьУсиление(p) {
    if (p.volSrc) { try { p.volSrc.disconnect(); } catch (e) {} }
    if (p.volGain) { try { p.volGain.disconnect(); } catch (e) {} }
    p.volSrc = null; p.volGain = null; p.volStream = null;
  }
  function собратьУсиление(p) {
    const поток = p.audio && p.audio.srcObject;
    if (!поток || !громче.сумма) return false;
    if (p.volGain && p.volStream === поток) return true;
    // Новый поток у того же собеседника (вышел из голоса при показе экрана и вошёл
    // снова): старая цепочка слушала бы пустой поток, а элемент заглушён — тишина.
    разобратьУсиление(p);
    p.volSrc = voiceCtx.createMediaStreamSource(поток);
    p.volGain = voiceCtx.createGain();
    p.volSrc.connect(p.volGain); p.volGain.connect(громче.сумма);
    p.volStream = поток;
    return true;
  }
  function applyPeerVolume(p) {
    ...
    // Элемент глушим, только когда усиленный звук действительно слышен, и НЕ
    // убираем: без живого элемента Chrome не держит дорожку peer-соединения, и звук
    // пропадает вовсе. Пока усиление не готово — обычный путь на полной громкости:
    // не громче, но слышно. Тишина была бы хуже.
    p.audio.muted = слышно;
    if (!слышно) p.audio.volume = 1;
  }

  function applyVoiceVolume() {
    if (voiceVolume > 1) {
      // Петлю собираем, только когда есть кого слушать: при открытии доски с
      // сохранённой громкостью выше 100% ни разговора, ни собеседников ещё нет.
      if (естьСлушать()) громчеГотовить();
    } else {
      громчеВыход('нет');
      громчеРазобратьПозже();
    }
  ...
  (function initVoice() {
    ...
    window.addEventListener('pagehide', () => {
      if (voiceOn) { try { rtcSend('bye', null, null); } catch (e) {} closeAllPeers(); }
      петляЗакрыть();
    });
    // Разрешение на звук браузер даёт по любому действию, не только по щелчку:
    // палец по доске, клавиша, возврат на вкладку.
    document.addEventListener('pointerup', звукРазбудить, true);
    document.addEventListener('keydown', звукРазбудить, true);
    document.addEventListener('visibilitychange', () => { if (!document.hidden) звукРазбудить(); });
    window.addEventListener('online', петляЕщё);
  })();
