...
(function () {
  ...
  // На сколько частей крупная клетка делится мелкой — как в Miro.
  const GRID_SUB = 4;
  ...
  function drawGrid(ctx) {
    ...
    // boardBg === 'grid': два уровня — крупная клетка и внутри неё мелкая 4×4.
    // Мелкая помогает попадать на глаз, крупная держит структуру и не даёт
    // полотну превратиться в однородную рябь. Осей нет: доска не координатная.
    const MAJOR = GRID * GRID_SUB;
    ctx.lineWidth = lw;
    ctx.strokeStyle = gc;

    // Мелкая — бледнее. Рисуем по всем узлам, включая крупные: сверху их
    // перекроет крупная линия, так проще и быстрее, чем пропускать каждую N-ю.
    ctx.save();
    ctx.globalAlpha = 0.4;
    ctx.beginPath();
    ...
    // Крупная — в полную силу.
    const majX = Math.floor(x0 / MAJOR) * MAJOR, majY = Math.floor(y0 / MAJOR) * MAJOR;
    ctx.beginPath();
    for (let x = majX; x <= x1; x += MAJOR) { ctx.moveTo(x, y0); ctx.lineTo(x, y1); }
    for (let y = majY; y <= y1; y += MAJOR) { ctx.moveTo(x0, y); ctx.lineTo(x1, y); }
    ctx.stroke();
  }
