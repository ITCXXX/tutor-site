...
class ОтсеченияВКорне(SimpleTestCase):
    """Отсечения в корне обязаны ускорять перебор, НЕ меняя выбор хода.

    Внутри дерева окно сужалось всегда, а в корне каждому ходу давалось полное
    окно, то есть альфа-бета там не работала вовсе. Правка передаёт вниз
    лучшее найденное значение — но только вместе с перепроверкой, и вот
    почему.

    Сужение даёт точную оценку лишь ходам ХУЖЕ текущего лучшего. Ход, который
    оказался лучше или сравнялся, обрывается на первом же доказательстве и
    возвращает границу. Сравнивать границы между собой нельзя: из двух хороших
    ходов корень выберет тот, чья граница случайно выше, а ход, который на деле
    хуже, может вернуть границу ровно вровень с лучшим и попасть в жеребьёвку
    равных.

    Это не рассуждение, а дважды измеренная ошибка. Без перепроверки вовсе бот
    проиграл сам себе 41.0% ± 4.8% на семёрке (400 партий). С перепроверкой
    только превысивших — один выбор из ста всё ещё расходился. Перепроверка
    превысивших И сравнявшихся: 140 позиций из 140 совпали.

    Поэтому тест берёт позиции пачкой и на двух уровнях: ошибка живёт там, где
    порядок ходов неудачен, и на одной-двух позициях не показывается.
    """

    def _уровень(self, имя, **правки):
        свой = dict(bot.LEVELS[имя])
        свой.update(правки)
        return свой

    def test_выбор_хода_не_меняется(self):
        for имя, сколько in (('medium', 12), ('hard', 6)):
            сужая = self._уровень(имя, корневое_окно=True)
            полным = self._уровень(имя, корневое_окно=False)
            for зерно in range(сколько):
                состояние = arena._случайный_дебют(зерно, n=5)
                сторона = состояние['turn']
                # Одинаковое зерно у обоих: выбор среди равных ходов случаен, и
                # без этого тест ловил бы монетку, а не правку.
                с_отсечениями = bot.choose_move(состояние, сторона, сужая,
                                                random.Random(7))
                полным_окном = bot.choose_move(состояние, сторона, полным,
                                               random.Random(7))
                self.assertEqual(с_отсечениями, полным_окном,
                                 'уровень %s, дебют %d' % (имя, зерно))
