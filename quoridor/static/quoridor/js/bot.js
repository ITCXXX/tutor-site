...
export const LEVELS = {
  ...
  medium: {
    ...
    // (800 партий). На девятке ход занимает 0.16 с при сроке 0.30 — уровень
    // успевает досчитаться целиком, значит выигрыш доедет и до браузера.
...
export function botMove(state, side, levelName = DEFAULT_LEVEL) {
  ...
  // каждому ходу долго давалось полное окно -Infinity..+Infinity — то есть на
  // верхнем уровне альфа-бета не работала вовсе, и заведомо худший ход
  // досчитывался целиком. Теперь лучшее найденное значение уходит вниз
  // потолком для соперника: ход, который уже хуже, обрывается на первом же
  // опровержении. Замер на сервере: тот же выбор хода в каждой проверочной
  // позиции, а времени 42% от прежнего.
  //
  // ПЕРЕПРОВЕРКА ОБЯЗАТЕЛЬНА. Точную оценку сужение даёт только ходам ХУЖЕ
  // текущего лучшего. Ход, оказавшийся ЛУЧШЕ, обрывается на первом же
  // доказательстве превосходства и возвращает лишь нижнюю границу; сравнивать
  // такие границы между собой нельзя. Без перепроверки бот проиграл сам себе
  // 41.0% ± 4.8% на семёрке — значит, «тот же выбор хода» был неправдой.
  // Поэтому превысивший считается заново полным окном; отсечения остаются на
  // заведомо худших ходах, а их большинство.
  // Сверка с сервером идёт через botView, где окно осталось полным.
  let scored = [];
  let order = pool;
  for (let depth = 1; depth <= level.depth; depth += 1) {
    ...
    for (const mv of order) {
      ...
      const next = apply(state, side, mv);
      if (!next) continue;
      const narrow = alpha !== -Infinity;
      let score = -search(next, other(side), depth - 1,
                          -Infinity, narrow ? -alpha : Infinity, level, budget);
      // Пересчитываются и те, кто СРАВНЯЛСЯ с лучшим: при узком окне ход,
      // который на деле хуже, возвращает верхнюю границу, и она может
      // оказаться ровно равной лучшему — такой ход попал бы в жеребьёвку
      // равных. Замер поймал это: один выбор из ста расходился.
      if (narrow && score >= alpha) {
        score = -search(next, other(side), depth - 1,
                        -Infinity, Infinity, level, budget);
      }
