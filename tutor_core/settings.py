...
# Логины у людей могут совпадать, поэтому вход идёт по ПАРЕ «логин + пароль».
# Обычный ModelBackend тут не годится: он достаёт одного пользователя по логину
# и на повторе падает прямо на странице входа. Подробности и правила отказа — в
# самом файле users/auth_backends.py.
AUTHENTICATION_BACKENDS = [
    "users.auth_backends.ПовторяющиесяЛогины",
]

# Django предупреждает (auth.W004), что USERNAME_FIELD не уникален, и подсказывает
# «убедитесь, что ваш способ входа это переживёт». Убедились: он для того и
# написан. Глушим предупреждение осознанно — висящее предупреждение, которое все
# привыкли пропускать, хуже, чем отсутствующее: рядом с ним не заметят настоящего.
