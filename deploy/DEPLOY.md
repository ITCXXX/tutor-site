...
На **уже работающем** сервере конфиг переписан certbot-ом, поэтому проверьте
вручную, что в нём есть блоки `location = /media-guard/` и
`location ~ ^/media/(hw/submissions|materials/files)/`. Их можно скопировать из
