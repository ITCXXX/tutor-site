...
`TURN_TTL` — срок пропуска к ретранслятору в секундах (по умолчанию 3600, то
есть час). Страница доски теперь сама просит новый пропуск, пока открыта, так
что часа хватило бы; восемь часов стоят страховкой для вкладок, открытых до
этой правки: у них список серверов берётся один раз, и через час ретранслятор
начинал отказывать («check_stun_auth: Cannot find credentials» в его журнале),
а голос пропадал у того, кому прямая связь не проходит.
