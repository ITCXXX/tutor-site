...
class Command(BaseCommand):
    ...
    def handle(self, *args, **opts):
        ...
        # Удаляем только сиротские записи с order < 1 (старый «order=0»).
        # Не трогаем order > len(PROTOTYPES): эти заняты seed_oge10_new
        # (новые типы 9-18). Если хочешь полный сброс — используй --clear.
