...
class User(AbstractBaseUser):
    # Основные поля
    # НЕ уникальный, и это осознанно: логины ученикам заводит преподаватель, и
    # удобно давать простые — «полина», «ваня». Пока стояло unique=True, второй
    # Полине приходилось выдумывать «полина2», а потом вспоминать, кто из них
    # кто. Различает людей теперь пара «логин + пароль»
    # (users/auth_backends.py).
    #
    # db_index обязателен. Раньше индекс появлялся сам, как побочный эффект
    # unique=True; сняв уникальность, его легко потерять — а по логину ищут при
    # каждом входе, и без индекса это чтение всей таблицы.
    username = models.CharField('Логин', max_length=50, db_index=True)
